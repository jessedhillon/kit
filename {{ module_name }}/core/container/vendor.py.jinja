...
class GoogleContainer(DeclarativeContainer):
    @staticmethod
    def provide_creds(project_id: str, private_key_id: str, private_key: p.Secret[str], client_email: p.EmailStr,
                      client_id: str) -> google.oauth2.service_account.Credentials:
    ...
    @t.overload
    @staticmethod
    def provide_service(credentials: google.oauth2.service_account.Credentials, service_name: t.Literal["sheets"],
                        service_version: t.Literal["v4"]) -> SheetsResource.SpreadsheetsResource: ...

    @t.overload
    @staticmethod
    def provide_service(credentials: google.oauth2.service_account.Credentials, service_name: str,
                        service_version: t.Literal["v4"]) -> googleapiclient.discovery.Resource: ...

    @staticmethod
    def provide_service(credentials: google.oauth2.service_account.Credentials, service_name: t.Literal["sheets"] | str,
                        service_version: str) -> googleapiclient.discovery.Resource:
    ...
    credentials: Provider[google.oauth2.service_account.Credentials] =\
        Singleton(provide_creds,
    ...
    sheets: Provider[SheetsResource.SpreadsheetsResource] =\
        Singleton(provide_service, service_name='sheets', service_version='v4', credentials=credentials)
...
class VendorContainer(DeclarativeContainer):
    ...
    google: Provider[GoogleContainer] = Container(GoogleContainer,
                                                  config=config.google,
                                                  secrets=secrets.google)
