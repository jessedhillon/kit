...
import testproject.lib.json as json
import testproject.lib.uuid as uuid
from testproject.lib.sql import DebugQuery, DebugSession
...
def provide_fixtures(root: Path | NotReady) -> types.ModuleType:
    ...
    fp = root / "fixtures"
    spec = importlib.util.spec_from_file_location("testproject_fixtures", os.path.join(fp, "__init__.py"))
    if not (spec and spec.loader):
        raise RuntimeError(f"could not provide fixture loader: {fp}")

    module = importlib.util.module_from_spec(spec)
    sys.modules["testproject_fixtures"] = module  # register in sys.modules
