...
import testproject.lib.uuid as uuid

# This module is a thin wrapper around Click, which is why we import `click.*`
# into our namespace. We create a class `TestCommand`. On invocation, we
# inject the `TestContainer` at runtime and add key artifacts from
# our current context into the container:
#
#   - `TestContainer.command`: a friendly name for the invoked command
#   - `TestContainer.contexts`: a list of command names and arguments which
